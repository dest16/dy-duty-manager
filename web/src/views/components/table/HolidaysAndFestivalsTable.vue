<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="日期" min-width="100" fixed prop="date" />
      <el-table-column label="假日名称" min-width="120" prop="name" />
      <el-table-column label="倍数计划" min-width="350">
        <template slot-scope="scope">
          <div v-html="formatPlan(scope.row.festival_plan)" />
        </template>
      </el-table-column>
...
import HolidaysAndFestivalsForm from '@/views/components/form/HolidaysAndFestivalsForm'
import { ArrayToDictionary, RetainTheDecimal } from '@/views/common/js/dataConversion'

export default {
  name: 'HolidaysAndFestivalsTable',
  components: {
    HolidaysAndFestivalsForm
  },
  data() {
  ...
  methods: {
    // plan字段以字符串对象（字典）形式保存，要格式化
    formatPlan(plan) {
      try {
        let res = ''
        plan.forEach(item => {
          // 津贴、补贴等总费用为0的值班类型不显示
          if (item.totalAllowance + item.totalSubsidizedMeals > 0) {
            const dc = this.dutyClassifyDict[item.classify]
            const dc_dom = '<span class="department-label" style="background: ' + dc.color + '">' + dc.label.slice(0, 1) + '</span>'
            const ms_list = []
            // 默认都是1倍，因此如果是1倍，则也无需显示
            if (item.totalAllowance > 0) ms_list.push(' 津贴 ×' + item.allowanceMultiple + ' = ' + item.totalAllowance)
            if (item.totalSubsidizedMeals > 0)ms_list.push(' 餐补 ×' + item.subsidizedMealsMultiple + ' = ' + item.totalSubsidizedMeals)
            const sum = item.totalAllowance + item.totalSubsidizedMeals
            const sum_dom = '<em style="font-weight: bold;color: #F56C6C">' + RetainTheDecimal(sum, 2) + '</em>'
            if (ms_list.length > 0) res = res + '<div>' + dc_dom + ms_list.join('，') + '，合计￥' + sum_dom + '元' + '</div>'
          }
        })
        return res
      } catch (e) {
    ...
    async fetchData() {
      ...
      //   HolidaysAndFestivals.getList(params),
    ...
    actionObject(action, row) {
      if (action === 'D') {
        ...
        if (action === 'A') {
          ...
          this.dialogTitle = '新增【假日】'
        } else if (action === 'E') {
          this.formData = row
          this.dialogTitle = '编辑【假日】'
        }
    ...
    async deleteConfirm(id, label) {
      ...
      const fullLoading = Loading.service({ text: '请稍后...', background: 'rgba(0, 0, 0, 0.7)' })
      const result = await singleRequest(HolidaysAndFestivals.remove(id))
    ...
    batchActionObject(action) {
      if (action === 'D') {
        ...
        let label
        let count_label = ''
        label = this.tableSelected.map(item => item.date + '（' + item.name + '）').join('、')
    ...
    async batchDeleteConfirm(label, count_label) {
      ...
      const result = await singleRequest(HolidaysAndFestivals.batchDelete(submitData))
      // 以“服务”的方式引入的loading需要异步关闭
      this.$nextTick(() => { fullLoading.close() })
      // 请求处理
